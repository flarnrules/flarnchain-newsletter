...
start_y = HEIGHT - 60  # Starting a bit higher to accommodate blocks
...
def draw_window():
    ...
    for i, block in enumerate(blocks):
        ...
        if i > 0:  # To ensure we don't try to draw a line for the first block
            pygame.draw.line(win, BLACK, (block['rect'].x + block_width // 2, block['rect'].y), (block['rect'].x + block_width // 2, block['rect'].y - block_space), 3)
